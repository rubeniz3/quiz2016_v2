...
exports.index = function(req, res, next) {
  ...
  if (search ===""){
    ...
    models.Quiz.findAll({where: {question: {$like: "%"+search+"%"}}, include: [ models.Attachment ] }).then(function(quizzes) {
      res.render('quizzes/index.ejs', {quizzes: quizzes});
  })
    .catch(function(error) { next(error); });
  }
...
exports.update = function(req, res, next) {
  ...
  req.quiz.save({fields: ["question", "answer"]}).then(function(quiz) {
    ...
    // Salvar la nueva imagen en Cloudinary
    return uploadResourceToCloudinary(req).then(function(uploadResult) {
...
function createAttachment(req, uploadResult, quiz) {
  ...
  .catch(function(error) { // Ignoro errores de validación en imagenes
...
// Crea una promesa para actualizar un attacment en la tabla Attachments
//
 ...
 * También borra la imagen original.
 *
 * Si puede subir la imagen la promesa se satisface y devuelve el public_id y la url del recurso subido.
 * Si no puede subir la imagen, la promesa también se cumple pero devuelve null.
 */
